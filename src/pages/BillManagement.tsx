...
import { Loader2, Plus, Filter, Search, Zap, TrendingUp, TrendingDown, DollarSign, AlertTriangle } from 'lucide-react';
...
const BillManagement = () => {
  ...
  // Fetch financial summary to get total expenses for analytics
  const { comparison, isLoading: isFinancialSummaryLoading } = useFinancialSummary();
  ...
  const handleFormSubmit = async (data: BillFormValues) => {
    ...
    try {
      if (editingBill?.id) {
        const { error } = await supabase.from('essential_bills').update(billData).eq('id', editingBill.id);
        if (error) throw error;
        showSuccess('Bill updated successfully!');
      } else {
        const { error } = await supabase.from('essential_bills').insert(billData);
        if (error) throw error;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('essential_bills').delete().eq('id', id);
      if (error) throw error;
  ...
  const { totalCurrentMonthBills, pendingBills, overdueBills, billChangePercent, trendIcon, trendColor } = useMemo(() => {
    const currentMonthStart = startOfMonth(new Date());
    const lastMonthStart = startOfMonth(subMonths(new Date(), 1));

    const currentMonthBills = bills.filter(b => new Date(b.due_date) >= currentMonthStart);
    const lastMonthBills = bills.filter(b => new Date(b.due_date) >= lastMonthStart && new Date(b.due_date) < currentMonthStart);
    ...
    const overdueBills = pendingBills.filter(b => isPast(new Date(b.due_date)));
    ...
    return { totalCurrentMonthBills, pendingBills, overdueBills, billChangePercent, trendIcon, trendColor };
  }, [bills]);

  if (isSessionLoading || isLoading || isFinancialSummaryLoading) {
    return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <SummaryCard title="Bills This Month" value={formatCurrency(totalCurrentMonthBills)} icon={DollarSign} colorClass="text-blue-500" delay={0.1} />
        <SummaryCard title="Pending Bills" value={pendingBills.length} icon={Filter} colorClass="text-yellow-500" delay={0.2} />
        <SummaryCard title="Overdue Bills" value={overdueBills.length} icon={AlertTriangle} colorClass="text-destructive" delay={0.3} />
        <SummaryCard title="Monthly Trend" value={`${billChangePercent.toFixed(1)}%`} icon={trendIcon} colorClass={trendColor} delay={0.4} />
      </div>
      ...
      <Card className="p-4 flex flex-col md:flex-row gap-4 justify-between items-center">
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          ...
          <DialogContent className="sm:max-w-[600px]">
            ...
            <BillForm initialData={editingBill} onSubmit={handleFormSubmit} isSubmitting={isSubmitting} />
          </DialogContent>
        </Dialog>
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div className="relative w-full sm:w-48">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Search title/desc..." className="pl-9" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
      ...
      {filteredBills.length === 0 ? (
        <div className="text-center p-10 text-muted-foreground border rounded-lg">No bills found matching the current filter.</div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <AnimatePresence initial={false}>
            {filteredBills.map((bill) => (
              <BillItem key={bill.id} bill={bill} onEdit={handleEdit} onDelete={handleDelete} onMarkPaid={handleMarkPaid} />
            ))}
          </AnimatePresence>
        </div>
      )}
      
      <BillAnalytics bills={bills} currentMonthTotalExpenses={comparison.currentMonthExpenses} />
    </div>
  );
};
